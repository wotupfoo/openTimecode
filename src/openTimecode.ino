...
#if 0
			bool isLastLTCbit = ( tick_count_32kHz > TICK_32kHz - constLTC_sub_32kHzTicks );
			bool isNewLTCbit  = ( tick_count_32kHz - tick_count_32kHz_prev >= constLTC_sub_32kHzTicks );

		 // IT SHOULD BE IMPOSSIBLE TO HAVE SKID SINCE IT'S THE SAME CLOCK
		// Is it the last subsample?
		if( isLastLTCsubsample ) {
			// Once per hour...
			// Check at "hh:59:59.[fps-1]" to see if there is a skid
			if( /*(RTCtime.getMinute() == 59) &&*/ (RTCtime.getSecond() == 59) ) {
				snprintf(oled_buff, OLED_BUFF_MAX,"ORIG   %02d:%02d:%02d.%02d,%d %5d %5d",
							RTCtime.getHour(), RTCtime.getMinute(), RTCtime.getSecond(), LTCframe,LTCbit,
							tick_count_32kHz,
							tick_count_32kHz_prev
							);
				Serial.println(buff);
				// Update from RTC
				DS3231_get(&t);
				if( /*(RTCtime.getMinute() == 59) &&*/ (RTCtime.getSecond() == 59) ) {
					snprintf(oled_buff, OLED_BUFF_MAX,"ONTIME %02d:%02d:%02d.%02d,%d %5d %5d",
							RTCtime.getHour(), RTCtime.getMinute(), RTCtime.getSecond(), LTCframe,LTCbit,
							tick_count_32kHz,
							tick_count_32kHz_prev
							);
					Serial.println(buff);
					// Wait for a 1Hz tick
//          while( xSemaphoreTake(sem1Hz,0) == 0 );
				} else if ( /*(RTCtime.getMinute() == 0) &&*/ (RTCtime.getSecond() == 0) ) {
#warning DEAL WITH TIME SKID
					// We're running LATE
					snprintf(oled_buff, OLED_BUFF_MAX,"LATE   %02d:%02d:%02d.%02d,%d %5d %5d",
							RTCtime.getHour(), RTCtime.getMinute(), RTCtime.getSecond(), LTCframe,LTCbit,
							tick_count_32kHz,
							tick_count_32kHz_prev
							);
				}
			} // OLD-NEW Time matching
		} // isLastLTCsubsample

#if 0 // Don't need this right now so save some cycles
			bool isNewLTCsec = (LTCframe == 0);
			if( isNewLTCsec ) {
				RTCtime.setSecond(++RTCtime.second() %= 60);
				// MINUTES
				bool isNewLTCmin = (RTCtime.second() == 0);
				if( isNewLTCmin ) {
					RTCtime.setMinute(++RTCtime.minute() %= 60);
					// HOURS
					bool isNewLTChour = (RTCtime.minute() == 0);
					if( isNewLTChour ) {
						RTCtime.setHour(++RTCtime.hour %= 24);
					}
				}
			}
...
#ifdef ENABLE_NTP
// --------------------------------------------
// NTP TASK - Deal with getting a new NTP update and applying to local RTC
// --------------------------------------------
void ntp_task() {
		static struct ts timecode;

		NTPtimeClient.update();
		Serial.println( timeClient.getFormattedTime() );

		// UPDATE SMTPE TIMECODE BASED ON NETWORK TIME
		timecode.years = 20;
		timecode.months = 9;
		timecode.days = (unsigned char)NTPtimeClient.getDay();
		timecode.hours = (unsigned char)NTPtimeClient.getHours();
		timecode.mins = (unsigned char)NTPtimeClient.getMinutes();
		timecode.secs = (unsigned char)NTPtimeClient.getSeconds();
#if 0
		timecode.timezone[0] = timeClient.timezone[0];
		timecode.timezone[1] = timeClient.timezone[1];
		timecode.timezone[2] = timeClient.timezone[2];
		timecode.timezone[3] = timeClient.timezone[3];
		timecode.timezone[4] = timeClient.timezone[4];
		timecode.timezone[5] = timeClient.timezone[5];
#endif

	// Update the RTC (DS3231)
	// TODO
	 
	// Update the LTC structure
	ltc_encoder_set_timecode(LTCencoder, &timecode);
}
#endif // ENABLE_NTP
#endif // ENABLE_WIFI
